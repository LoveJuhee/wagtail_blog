...
class crawl_naver_webtoon:
    ...
    def get_episode_lists(self, titleid, latest_db_no=0):
        ...
        while has_nextpage:
            ...
            latest_ep_no = (int)(re.findall(ep_no_pattern, title_lists[0].attrs.get('href'))[1])
            if (int)(latest_db_no) >= latest_ep_no:
                print("신규 에피소드가 없습니다.")
                return False;
            
            if page <= 1:
                webtoon_title_name = dom.select('.comicinfo h2')[0].text.split()[0]
            latest_ep_title = title_lists[0].text

            episode_lists = []
            episode_lists.append((webtoon_title_name, latest_ep_no, latest_ep_title))
            
            for title in title_lists:
                episode_no   = (int)(re.findall(ep_no_pattern, title.attrs.get('href'))[1])
    ...
    def update_db(self, webtoon_in_db, webtoon_info):
        ...
        if not webtoon_in_db:
            query = "insert into tb_cr_webtoon(titleid, latest_ep_no, episode_title, webtoon_title) " + \
                    "values (%s, %s, %s, %s)"
            data.append( (self._webtoon_titleid, webtoon_info[1], webtoon_info[2].replace("'", '`'), webtoon_info[0],) )
        elif webtoon_in_db:
            query = "update tb_cr_webtoon set latest_ep_no = %s, episode_title = %s where titleid = %s"
            data.append( (webtoon_info[1], webtoon_info[2], self._webtoon_titleid,) )
...
def main(user_input_title, user_input_assigned_no = 0, the_episode_only=None ):
    ...
    if not db_result:
        if webtoon._webtoon_titleid:    # DB에 등록되지 않은 웹툰은 타이틀id 값이 필수
            episode_lists = webtoon.get_episode_lists(webtoon._webtoon_titleid, webtoon._assigned_no)
        else:
